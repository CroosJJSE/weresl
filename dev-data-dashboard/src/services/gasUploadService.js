/**
 * Google Apps Script (GAS) Upload Service for Dev Data Dashboard
 * Handles image uploads to Google Drive via GAS
 */
 ...
 */
export const uploadToDrive = async (file, regId = null, retryCount = 0) => {
  try {
    // Validate file
    ...
    // Generate filename with Reg_ID if provided
    const timestamp = Date.now();
    const filename = regId ? `${regId}_${timestamp}.jpg` : `profile_${timestamp}.jpg`;
    ...
    if (retryCount < 3 && (error.message.includes('5') || error.message.includes('network'))) {
      ...
      return uploadToDrive(file, regId, retryCount + 1);
    }
 ...
 */
export const uploadImageWithProgress = async (file, regId = null, onProgress) => {
  try {

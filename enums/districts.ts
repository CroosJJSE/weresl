...
export const DISTRICT_MAPPING: Record<District, string> = {
...
// Helper functions
export const getDistrictName = (code: District): string => {
  return DISTRICT_MAPPING[code] || 'Unknown'
}

export const getDistrictCode = (name: string): District | null => {
  return DISTRICT_CODES[name] || null
}

export const getAllDistricts = (): Array<{ code: District; name: string }> => {
  return Object.entries(DISTRICT_MAPPING).map(([code, name]) => ({
    code: code as District,
    name
  }))
} 
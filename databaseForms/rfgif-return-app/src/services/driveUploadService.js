/**
 * Modular Google Drive Upload Service for RFGIF Return App
...
const DRIVE_CONFIG = {
  ...
  ALLOWED_TYPES: [
    ...
    'image/webp',
    'application/pdf',
    'application/octet-stream' // For some PDF files
  ]
};
...
export const validateFile = (file) => {
  ...
  // Check file type (now includes PDFs)
  if (!DRIVE_CONFIG.ALLOWED_TYPES.includes(file.type)) {
    errors.push('Please select a valid image file (JPEG, PNG, GIF, WebP) or PDF');
  }
...
export const uploadImageWithProgress = async (file, onProgress) => {
  try {
    ...
    onProgress && onProgress(10, 'Converting file...');
